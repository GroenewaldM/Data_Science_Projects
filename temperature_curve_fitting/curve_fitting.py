...
def generate_models(x, y, degs):
    ...
    coefficients = []
    for degree in degs:
        coefficients.append(pylab.polyfit(x, y, degree))
...
def r_squared(y, estimated):
    ...
    for i in range(len(y)):
        numerator += (y[i] - estimated[i]) * (y[i] - estimated[i]) 
...
def evaluate_models_on_training(x, y, models):
    ...
    for model in models:
        estimated = []
        for x_value in x:
            y_value = np.polyval(model, x_value)
            estimated.append(y_value)
        pylab.plot(x, estimated, '--', label = "Options")
        if r_squared(y, estimated) > R_squared:
            R_squared = r_squared(y, estimated)
            y_best_fit = estimated
    pylab.plot(x, y_best_fit, '-', label = "Best Fit")
    pylab.plt.xlabel('Date')
    pylab.plt.ylabel('Temperature')
